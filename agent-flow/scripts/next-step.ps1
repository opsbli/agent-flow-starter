...
function Analyze-Change {
    ...
    if ($flow -eq "Unknown") {
        ...
        if (Test-ListContains -List $missing -Value "REQUIREMENT.md") {
            ...
            $prompt = "Continue agent-flow change: $changeId. Based on DESIGN, complete TASKS.md. Each task must include goal, AC mapping, read_files, write_files, verification command, and parallelization status."
            ...
            $stage = "verify"
            $next = "Run verification and complete VERIFY.md."
            $prompt = "Continue agent-flow change: $changeId. Run the relevant verification commands and complete VERIFY.md with command log, AC evidence, drift checks, skipped checks, and conclusion."
            ...
            $prompt = "Continue agent-flow change: $changeId. Run Closure Audit. Check Closure Gates, VERIFY evidence, AC coverage, drift-check, undeclared file changes, and knowledge/decision/log/baseline updates."
