...
if ! grep -Eiq '\[x\][[:space:]]+Emergency' "$change_path"; then
...
meaningful() {
  ...
  ! printf '%s' "$value" | grep -Eiq 'TODO|TBD|\{.+\}|pending-user|your-name'
}
...
meaningful "$approved_by" || issues+=("Emergency Approved by must name an accountable approver.")
meaningful "$bypass_reason" || issues+=("Emergency Bypass reason must explain why the full flow was skipped.")
meaningful "$deadline" || issues+=("Emergency Backfill deadline must be set.")
