...
把下面这段发给 AI，三段式 + 流程指令一次搞定：

```text
按 agent-flow 流程处理这个需求：

场景：我在{项目类型}的{模块}做开发。
目标：需要{具体要完成的事}，不改{明确不碰的}。
风险：涉及{数据库/权限/API/状态机/跨模块}（没有就写无）。

先做 code-first 扫描，判断 Light/Standard/Heavy。
然后每次只问一个关键问题完成 Grill，
如果你能通过读代码回答就不要问我。
每个问题给出你的推荐答案。
```

**三段式不是可选步骤，它就是"按 agent-flow 流程处理"的标准输入格式。**

### 记不住怎么办？

三句话就够：

> 按 agent-flow 流程处理这个需求：`<一句话>`。先做 code-first 扫描，分级，然后每次只问一个关键问题。

### 为什么这有效

| 问题 | 这段 prompt 怎么解决的 |
|------|-----------------------|
| AI 不查代码直接写 | `code-first 扫描` 强制先读代码 |
| AI 猜你的意思 | 每次 `只问一个关键问题` + `给出推荐答案` |
| 信息过载 | 一次一个问题，不是一次性全问 |
| AI 假设你不懂技术 | 你的 `场景/目标/边界/风险` 让它知道你已经思考过 |
| 做完没证据 | Grill 对齐后走 Light/Standard/Heavy，验证门禁兜底 |
...
当你收到一个需求（无论是否按三段式格式），你的第一轮回复必须包含以下三个结构：

### 1. 重述理解

用一句话确认你对需求的理解。示例：

> **理解**：你需要在用户设置页面加一个「是否接收邮件通知」的开关，用现有的 user_profile 表，不改 schema，不改权限。

### 2. 确认分级

> **分级判断**：Standard —— 单模块功能，不改 schema 和权限。

如果用户没有给出风险信息，你必须通过读代码来推断分级，而不是直接问"你觉得是什么级别"。

### 3. 第一个 Grill 问题

> **第一个问题**：{问题}。我的推荐答案是：{答案}。
>
> （如果我能通过读代码回答，我就直接回答，不会问你。）

**禁止**：一次性问完所有问题、不加推荐答案直接问、或在分级前就直接编码。
...
- 门禁要求按流程分级定义在 `agent-flow/rules/gate-tiers.md` 中。
  包括但不限于以下门禁：design-check、alignment-check、task-check、plan-check、
  code-drift-check、coverage-check、blocked-check、task-boundary-check、
  emergency-check、manifest-check、evolution-check、closure-check。
