...
> ⚠️ **绑定指令**已移至 `.pi/APPEND_SYSTEM.md`（追加到系统提示词，模型视为行为指令）。
> AGENTS.md 现在只保留**参考性知识**（编辑规则、ECC 集成、安装契约等）。
>
> 首次使用本仓库时，请同时阅读：
> - `.pi/APPEND_SYSTEM.md` — 绑定指令（入口、流程纪律、12 条默认取向）
> - `agent-flow/README.md` — 流程详细文档
